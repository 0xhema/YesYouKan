...
const Header = () => {
  ...
  const { currentBoard, deleteBoard , boards } = useBoards();
  ...
  return (
    <header className="p-4 h-[85px] flex bg-white justify-between items-center border-b border-lightGreyLine dark:bg-darkGrey dark:text-white dark:border-darkGreyLine md:p-0">
        ...
        { !boards.length|| !currentBoard.columns.length?(<>
            <button className="btn btn__primary btn-sm align-end mr-4" onClick={async()=>{
            await firebase.auth().signOut();
            window.location.reload()
        }}>
                Sign Out
         </button></>):(<> <div className="flex items-center gap-4 md:pr-4">
